...
## 5 · What this bundle adds as code — `kotlin/`
See **[BUILD_ORDER.md](BUILD_ORDER.md)** for the full list and the order to add them. In short:

- `ui/theme/Dimens.kt` — the spacing scale the repo lacks (`Dim.*`). Everything else in `ui/theme` is untouched.
- `ui/theme/ForgeExpression.kt` — **new.** Heat / Energy / Surface as one `CompositionLocal`. `Heat.Ember` is
  exactly `AccentPrimary`, so the default state is the shipped look; the other two heats re-tint action only.
  `Energy.Calm` is reduce-motion. `SurfaceStyle` picks the hero treatment.
- `ui/components/ForgedMark.kt` + `res/drawable/ic_forged_mark.xml` — the barbell mark that replaces the spark.
- `ui/components/ForgedSurfaces.kt` — `forgeGround()` / `forgeHero()` / `ForgeHairline()` / `ForgeSectionHeader()`.
  The rule these enforce: **one hero per screen, everything else flat.** That is the fix for "all six tabs look alike".
- `ui/components/body/` — 35 named muscle groups (front + back) and the Canvas that tints them from
  `heat.at(freshness)`. This supersedes §6.2: the paths are now real anatomy, not the old abstract blobs.
- `ui/home/HomeHubScreen.kt`, `ui/session/SessionSlateScreen.kt` — the two screens as stateless composables
  taking a UI model, so all logic stays in your ViewModels.

Every click modifier in those files is commented out (`// .clickable(...).forgedPress()`) because your
`forgedPress` signature is the source of truth. Uncomment and wire.
...
2. ~~**Body map**~~ **RESOLVED.** The map was rebuilt as a real anatomical physique and now ships as
   `kotlin/ui/components/body/MuscleBodyPaths.kt` + `MuscleBodyMap.kt` — 35 named muscle groups, front and
   back, tinted at runtime from `heat.at(freshness)`, no % pills on the body (they stay in the BY MUSCLE list).
   Drop these into `ui/components/body/` in place of the `MuscleBodyMap.kt` sketch.
...
7. **Home has diverged — now resolved in code.** `kotlin/ui/home/HomeHubScreen.kt` is the decision:
   the Dynamic Hub hero replaces the old hero card, and `MuscleTargetFigure`'s "what's warm today" job moves
   into the **Ready to train** rail as heat-tinted muscle cards. The weekly ring, streak tile and greeting
   survive — greeting in the brand row, ring and streak as rail cards. Original conflict, for the record: The shipped Home (per `MEMORY.md` v5) leads with `MuscleTargetFigure` (170 dp
