...
- **Spacing** — the one gap; `tokens/Dimens.kt` in this bundle fills it.
...
## Assets
- **Barbell logo mark** — 5 rounded rects on a 32 × 32 grid, tinted with the accent; recreate as a vector
  drawable (spec: bar 9,14.25 14×3.5 r1.5 · inner plates 6.5/22, 9.5, 3.5×13 r1.6 · outer plates 3/26, 12, 3×8 r1.4).
...
- **Body maps** — `prototype/bodyPaths.js` (front/back muscle path data). Ship as two vector drawables with
  named muscle groups you can tint at runtime.
...
- `tokens/Dimens.kt` — the spacing scale the repo doesn't have yet (drop into `ui/theme/`).
